...
heartrate_new%>%
  filter(drug=="a")%>%
  summarise(mean_a=mean(heartrate))
```
...
```r
heartrate2%>%
  separate(patient,into = c("name","gender"),sep="_")%>%
  pivot_longer(a:d,
               names_to = "drug",
               values_to = "heartrate",
               values_drop_na =T)%>%
  arrange(desc(name,drug,heartrate))
```

```
## # A tibble: 24 x 4
##    name     gender drug  heartrate
##    <chr>    <chr>  <chr>     <dbl>
##  1 Trapper  m      a            60
##  2 Trapper  m      b            58
##  3 Trapper  m      c            64
