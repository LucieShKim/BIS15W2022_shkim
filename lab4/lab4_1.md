...
```r
anyNA.data.frame(mammals,max.life)
```
...
```r
mammals_new<-rename(mammals,genus="Genus",wean_mass="wean mass",afr="AFR",max_life="max. life",litter_size="litter size",litter_year="litters/year")
```
...
## #   litter_size <dbl>, litter_year <dbl>
```
...
```r
mass<-select(mammals_new,genus:mass)
mass
```
...
```r
mm_new<-select_all(mammals, ~str_replace(., " ", "_"))
mm_new
```
