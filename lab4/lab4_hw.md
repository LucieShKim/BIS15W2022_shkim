...
```r
filter(homerange,genus%in%c("alces"))
```
...
##    taxon  common.name       class order family genus species primarymethod N    
##    <fct>  <chr>             <chr> <fct> <chr>  <chr> <chr>   <chr>         <chr>
##  1 snakes western worm sna~ rept~ squa~ colub~ carp~ vermis  radiotag      1    
##  2 snakes eastern worm sna~ rept~ squa~ colub~ carp~ viridis radiotag      10   
##  3 snakes racer             rept~ squa~ colub~ colu~ constr~ telemetry     15   
##  4 snakes yellow bellied r~ rept~ squa~ colub~ colu~ constr~ telemetry     12   
##  5 snakes ringneck snake    rept~ squa~ colub~ diad~ puncta~ mark-recaptu~ <NA> 
##  6 snakes eastern indigo s~ rept~ squa~ colub~ drym~ couperi telemetry     1    
##  7 snakes great plains rat~ rept~ squa~ colub~ elap~ guttat~ telemetry     12   
##  8 snakes western ratsnake  rept~ squa~ colub~ elap~ obsole~ telemetry     18   
##  9 snakes hognose snake     rept~ squa~ colub~ hete~ platir~ telemetry     8    
## 10 snakes European whipsna~ rept~ squa~ colub~ hier~ viridi~ telemetry     32   
...
```r
snake_s<-select(snake,species,log10.hra)
```


```r
arrange(snake_s,log10.hra)
```
